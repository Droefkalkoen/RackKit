...
- `SDK_v4.6.0/` — a local, read-only copy of the Reason Rack Extension (Jukebox) SDK,
  ...
  copyleft software, so it must not be committed to this GPL-3.0 repo. The one tracked
  exception is `Documentation/acceptance_testing_checklist.txt`, retained as reference.
